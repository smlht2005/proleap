...
/*
* Plan 2a: procedureOnlyStart : procedureDivision EOF — 用於僅解析 PROCEDURE DIVISION 子字串時之進入點
*/
...
// Entry point for parsing only PROCEDURE DIVISION (Plan 2a: when full-file parse does not reach it)
procedureOnlyStart
...
// - author paragraph ----------------------------------
// 允許 commentEntry 或自由格式文字（如 saltyfish.chang.、2012年12月10日）

authorParagraph
    : AUTHOR DOT_FS (commentEntry | identificationFreeText)?
    ;

// - installation paragraph ----------------------------------

installationParagraph
    : INSTALLATION DOT_FS (commentEntry | identificationFreeText)?
    ;

// - date written paragraph ----------------------------------

dateWrittenParagraph
    : DATE_WRITTEN DOT_FS (commentEntry | identificationFreeText)?
    ;

// - date compiled paragraph ----------------------------------

dateCompiledParagraph
    : DATE_COMPILED DOT_FS (commentEntry | identificationFreeText)?
    ;

// - security paragraph ----------------------------------

securityParagraph
    : SECURITY DOT_FS (commentEntry | identificationFreeText)?
    ;

// - remarks paragraph ----------------------------------

remarksParagraph
    : REMARKS DOT_FS (commentEntry | identificationFreeText)?
    ;

// 識別部段落自由格式文字（如 saltyfish.chang.、2012年12月10日 上午 10:05:42）
identificationFreeText
    : (IDENTIFIER | DOT | DOT_FS | INTEGERLITERAL | NONNUMERICLITERAL | NUMERICLITERAL | LPARENCHAR | RPARENCHAR | PLUSCHAR | MINUSCHAR | COLONCHAR)+
    ;
...
fileControlClause
    ...
    | passwordClause
    | relativeKeyClause
    | acuCompressionClause
    | acuLockModeClause
    ;

// ACUCOBOL: ASSIGN TO DISK "quoted" or ASSIGN TO "quoted"
assignClause
    : ASSIGN TO? (
        (DISK | DISPLAY | KEYBOARD | PORT | PRINTER | READER | REMOTE | TAPE | VIRTUAL) (assignmentName | literal)?
...
// ACUCOBOL: RECORD KEY name = keylist (comma or space separated)
recordKeyClause
    : RECORD KEY? IS? qualifiedDataName (EQUALCHAR qualifiedDataName (COMMACHAR? qualifiedDataName)*)? passwordClause? (WITH? DUPLICATES)?
    ;

// ACUCOBOL: ALTERNATE RECORD KEY / ALTERNATE KEY, with optional = keylist
alternateRecordKeyClause
    : ALTERNATE RECORD? KEY? IS? qualifiedDataName (EQUALCHAR qualifiedDataName (COMMACHAR? qualifiedDataName)*)? passwordClause? (WITH? DUPLICATES)?
    ;
...
// ACUCOBOL-GT file control extensions
acuCompressionClause
    : WITH? COMPRESSION (COMPRESSION CONTROL VALUE integerLiteral)? (ENCRYPTION)?
    ;
...
screenSection
    : SCREEN SECTION DOT_FS (screenDescriptionEntry | acuScreenContinuation)*
    ;
...
screenDescriptionEntry
    : INTEGERLITERAL (FILLER | screenName)? (
        ...
        | screenDescriptionExceptionClause
    )* screenDescriptionEntryTerminator
    ;
...
screenDescriptionExceptionClause
    : EXCEPTION PROCEDURE identifier DOT_FS
    ;
...
dataIsSpecialNamesClause
    : IS SPECIAL_NAMES cobolWord cobolWord
    ;
...
// ACUCOBOL-GT GUI statements
acuCreateStatement
    : CREATE identifier acuGuiPropertyList? (HANDLE IN? identifier)?
    ;
...
acuInquireStatement
    : INQUIRE identifier acuGuiPropertyList?
    ;
...
acuGuiPropertyList
    : (acuGuiProperty (COMMACHAR? acuGuiProperty)*)
    ;
...
SIGNED_LONG
    : S I G N E D MINUSCHAR L O N G
    ;

SIGNED_SHORT
    : S I G N E D MINUSCHAR S H O R T
    ;
...
UNSIGNED_LONG
    : U N S I G N E D MINUSCHAR L O N G
    ;
...
IDENTIFIER
    : [a-zA-Z0-9\u4e00-\u9fff\u3400-\u4dbf\uff00-\uffef]+ ([-_]+ [a-zA-Z0-9\u4e00-\u9fff\u3400-\u4dbf\uff00-\uffef]+)*
    ;
...
// ACUCOBOL: match *> alone or *> rest-of-line (WS optional for *>2019/12/24 etc)
COMMENTLINE
    : COMMENTTAG (WS? ~('\n' | '\r')*)? -> channel(HIDDEN)
    ;
...
// 兜底規則：捕捉所有未匹配字元（如罕見 Unicode、特殊符號），送到 HIDDEN channel
OTHER_CHAR
    : . -> channel(HIDDEN)
    ;
